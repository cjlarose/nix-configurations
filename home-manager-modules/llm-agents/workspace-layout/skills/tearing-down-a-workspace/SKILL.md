...
- `git worktree remove`, never `rm -rf` — it refuses a dirty tree, so teardown
  doubles as a safety check, and git prunes its own admin files.
...
- Close the herdr space too. Nothing prunes it automatically.
