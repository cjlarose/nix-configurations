...
# Anything in the workspace that is NOT a worktree is reported rather than
# skipped. A verdict of SAFE is a statement about the whole directory, and a
# stray checkout or a directory of notes silently ignored here reads as
# permission to delete it.
for entry in "$ws"/* "$ws"/.[!.]*; do
  [ -e "$entry" ] || continue
  [ -d "$entry" ] && [ -e "$entry/.git" ] && continue
  echo "=== $(basename "$entry")"
  echo "    NOT A WORKTREE. Left out of every check below."
  echo "    VERDICT: REVIEW BY HAND before removing the workspace."
  status=1
done
...
for wt in "$ws"/*/; do
  ...
  # 1b. A nested worktree is invisible to every check above. `git status` does
  #     not list IGNORED paths, and .claude is ignored globally -- so the
  #     .claude/worktrees/ that the layout actively encourages for parallel
  #     agents reads as a perfectly clean tree. `git worktree remove` then
  #     deletes it wholesale: it refuses modified and untracked files, but not
  #     ignored ones. Uncommitted work in there is gone with no warning, and the
  #     OTHER repo is left with a prunable worktree entry.
  nested=$(find "$wt" -mindepth 2 -name .git -printf '%h\n' 2>/dev/null | head -20)
  if [ -n "$nested" ]; then
    echo "    NESTED WORKTREES inside this one:"
    printf '%s\n' "$nested" | sed 's/^/      /'
    echo "    VERDICT: NOT SAFE. Removing this worktree deletes those wholesale,"
    echo "      ignored-but-real files included. Tear them down first, from"
    echo "      whichever repo each belongs to."
    status=1
    continue
  fi
