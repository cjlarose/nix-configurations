...
happens, and a rename whose target already exists is refused outright rather
than nesting the worktree inside it. Renames run before the add, but they are
not atomic — if one fails partway the earlier ones stand, and the script says
which. Re-running after fixing the cause finishes the cascade.
