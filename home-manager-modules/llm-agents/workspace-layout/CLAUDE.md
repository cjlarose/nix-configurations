...
The space is created and closed by the workspace skills' own scripts, so there
is nothing to look up for either. **Use the `herdr` skill** for everything else
— splitting panes, starting agents — and do not improvise those from memory; the
installed binary is the authority and that skill knows its current syntax.

herdr will not invent topology on its own: by default it starts an agent in a
...
when the tree is dirty, so teardown doubles as a safety check. The herdr space
