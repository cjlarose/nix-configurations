...
# wiki; the two run side by side and the trial is judged on evidence. The one
# question it exists to answer is whether a TASK-shaped project is a useful unit
# of memory, which is what the workspaces marker below is for -- see
# ~/workspaces/HANDOFF.md and the `workspacesMarker` option's own description.
#
...
let
  ...
  # What the renderer must agree with, in the shape install-hooks emits it.
...
in
{
  options.cjlarose.llmAgents.aiMemory = {
    ...
    workspacesMarker = {
      enable = lib.mkEnableOption ''
        the ~/workspaces/.ai-memory.toml marker: one file that makes every
        per-task workspace directory its own ai-memory project, with recall
        spanning all of them.

        This is the whole experiment, so it is worth stating what it buys.
        ...
        same project. The task-keyed layout here is deliberately one cwd
        spanning N repos, so ai-memory's unit of memory is repo-shaped and this
        workflow's is task-shaped, and no setting dissolves that.
        project_strategy = "repo-root" does not help: it derives a project from
        the checkout the cwd is INSIDE, and here the cwd is ABOVE several
        checkouts.

        So: let basename($cwd) stand, and let the task BE the project. Writes
        stay task-scoped, reads span everything (see defaultGlobalRecall). The
        rejected alternative was pinning a single "cross-repo" project, which
        has no sprawl but collapses every multi-repo task into one
        undifferentiated namespace -- close to what the llm-wiki already is, and
        upstream ships a heal_catch_all_repo_paths migration precisely because
        catch-all scoping is a known hazard.

        The cost to watch during the trial is the other half of the question:
        project sprawl, and projects orphaned when tearing-down-a-workspace
        removes the worktree. `ai-memory curator` reports on cold pages and
        dangling cross-project links without an LLM, and --pinned spares
        anything durable from the decay sweep
      '';
      ...
      path = lib.mkOption {
        ...
        example = "workspaces";
        description = ''
          The directory the marker is written into, RELATIVE TO $HOME (this is
          a home.file target, which is always home-relative -- an absolute path
          here would be installed at $HOME/<the absolute path>).

          It is the root holding one directory per task. Marker resolution walks
          UP from the session's cwd to $HOME and takes the nearest marker, so
          one file here covers every task beneath it and no per-task file is
          needed.
        '';
      };
      ...
      defaultGlobalRecall = lib.mkOption {
        ...
        description = ''
          ...
          This is the half that makes task-shaped projects survivable. Knowledge
          from a task touching three repos genuinely is not about any one of
          them -- it is cross-cutting synthesis, which the llm-wiki already
          files as type/synthesis rather than under an entity page. Without
          this, every finished task would be a sealed box.
        '';
      };
    };
  ...
  config = lib.mkMerge [
    ...
    # --- the marker that makes a task a project ------------------------------
    ...
    (lib.mkIf (enabled && cfg.workspacesMarker.enable) {
      home.file."${cfg.workspacesMarker.path}/.ai-memory.toml".source =
        tomlFormat.generate "ai-memory-workspaces-marker.toml" ({
          workspace = cfg.workspace;
        } // lib.optionalAttrs cfg.workspacesMarker.defaultGlobalRecall {
