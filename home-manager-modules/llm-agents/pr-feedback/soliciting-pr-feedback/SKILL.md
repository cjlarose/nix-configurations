...
6. **Surface the URL — required, every time.** The open command prints `url: "http://…/session/<id>"`. Put that link at the TOP of your very next message to the user, on its own line — "Review it here: <url>" — before any summary. The user cannot open a page whose URL they never see; do NOT make them ask for it or dig it out of tool output. Re-surface it whenever it changes (e.g. after `--reopen`, or if you backgrounded the open and the link scrolled off).
   - **Take the link from the environment; never compose one yourself.** How the URL is spelled is the *host's* decision, not yours. `programs.lavish` puts it in the session environment — `LAVISH_AXI_LINK_HOST` (the name to print), `LAVISH_AXI_LINK_SCHEME` (`https` when a TLS reverse proxy fronts it), `LAVISH_AXI_LINK_PORT` (empty string means omit the port, for a proxy on 443), and `LAVISH_AXI_PORT` (what lavish actually binds) — and lavish prints exactly the URL the *server's* copy of them implies. Run `env | grep LAVISH_AXI` once, not to build a URL out of the pieces but to tell the two cases below apart, then paste what lavish printed, verbatim.
     - **It printed `127.0.0.1`** → nothing is configured for off-machine access (`LINK_HOST` falls back through `LAVISH_AXI_HOST` to loopback). That URL is correct on this machine and unreachable from anywhere else. If the user is reviewing from another device, don't paste it and don't invent a hostname to patch it up — tell them the host has no link host configured and let them decide. Judge this on what lavish actually printed, not on which variables you found set: `LAVISH_AXI_HOST` alone is enough to produce a reachable link.
     - **Set in the config but absent from your shell** (it started before the config was deployed) → read the values out of the host's `programs.lavish` block, then run `lavish-axi stop` before opening with them exported. Exporting alone is not enough: the link variables are read in the *server* process and fixed when it spawns, and an open hands back any healthy server already on the port without comparing environments — so setting them around an open that reuses a server changes nothing and lavish goes on printing loopback. Of the four, only `LAVISH_AXI_PORT` still has to agree on a later poll; see the port gotcha below. **But `stop` is host-wide** — one server carries every session on the machine, so it also drops every other agent's review, and a poll of theirs caught mid-delivery loses that batch exactly the way a truncating filter would, to someone who will never learn why. Run a bare `lavish-axi` first; it lists `sessions[N]{file,status,url,pending_prompts}` — judge by which sessions are open, not by `pending_prompts`, since a poll caught mid-delivery shows nothing there. If anyone else has a review open, do not stop the server on your own initiative, and do not quietly settle for the loopback URL either: tell the user a reachable link needs the shared server restarted, that restarting it would drop the other reviews in progress, and let them decide — the same call the bullet above leaves to them.
