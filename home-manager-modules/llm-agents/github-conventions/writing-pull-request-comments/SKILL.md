...
|---|---|
| Commit message | **Required** — one `Assisted-by: <model>` trailer per model |
| PR description | Per repo — see **writing-pull-request-descriptions** |
| GitHub comment | **Required** — visible `*Generated-By: <model>*` signature |

The form follows the artifact: a trailer block belongs to a commit message and
is parsed there, prose belongs in a PR body, and an italic signature reads as a
signature inline in a discussion. None of them carries an email — a model is not
someone you can write to.
...
- Using a commit trailer form, `Assisted-by:` or otherwise, on a comment.
